public class Main {
    static void main() {
        ...
        int before = orange.getCount();
        try {
            orange.removeStock(10000);
        } catch (IllegalArgumentException e) {
            System.out.println("argument removeStock must be value <= count");
        }
        int after = orange.getCount();
        System.out.println("Checking atomicity before and after exclusion\n" +
                "The values of the count fields before and after the exclusion are equal:");
        System.out.println(before == after);

        try {
            orange.addStock(Integer.MAX_VALUE);
        } catch (IllegalArgumentException e) {
